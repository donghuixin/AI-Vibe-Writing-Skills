...
5. **Draft**: 根据用户输入的主题进行撰写，并在需要处嵌入证据。
6. **Audit**: 这是一个自我反思步骤。检查生成的内容是否包含“AI 味”过重的词（如 "crucial", "game-changer"），或者是否犯了错题本里的错误。如果发现，立即修正。
7. **Evidence Check**: 核验证据使用是否符合 Evidence Requirements，不足则补充或标注缺口。
